...
do
  ...
  if [ "$ver" == "7" ]
  then
    grep -a -A $[ Nroots + 30 ] 'Dipole Trans Strengths (SO states)' $log | gawk "/  1 /" >tmp2
  elif [ "$ver" == "8" ]
  then
    grep -a -A $[ Nroots + 30 ] 'Dipole transition strengths (SO states):' $log | gawk "/  1 /" >tmp2
  fi
...
#then
#  echo "Usage:"
#  echo "./extract_molcas.sh {general_name} {number of files} {molcas version}"
#else
